<!--

<template>
    <div id="nav-container" class="light-bg dark-text">
        <nav id="navbarMenu">
        <div class="toggleMenu">JaID</div>
            <ul id="nav-links">
                <li><a target="_self" href="/">Home</a></li>
                <li v-for="category in data.categories" :key="category.name" class="submenu"><a>{{ category.name }}</a>
                    <ul>
                        <li><a target="_self" class="dark-bg light-text" v-bind:href="category.link">all {{ category.name }}</a></li>
                        <li v-for="item in category.items" :key="item.title"><a target="_self" class="dark-bg light-text" v-bind:href="item.link">{{ item.title }}</a></li>
                    </ul>
                </li>
                <li><a>Contact</a></li>
                <li><a>About</a></li>
                <a target="_self" class="nav-logo dark-text" href="/">JaID</a>
            </ul>
        </nav>
        <br class="mobile-br">
    </div>
</template>

-->




<template>

    <div id="header">

        <a href="#" id="hamburger">
            <div>
                <i class="fas fa-bars"></i>
            </div>
        </a>

        <div id="header-brand">
            <a href="/">
                <div id="brand-title">
                    <h1>yosemite</h1>
                </div>
                
                <div id="brand-subtitle">
                    <p>open e-commerce by Jaiden DeChon</p>
                </div>
            </a>
        </div>

        <div id="nav">

            <ul id="nav-links-left">

                <li><a href="/">Home</a></li>

                <li v-for="category in data.categories" :key="category.name" class="has-dropdown"><a href="javascript:void(0)">{{ category.name }}</a><i class="fas fa-angle-down"></i>

                <ul class="dropdown">

                    <li><a v-bind:href="category.link">all {{ category.name }}</a></li>

                    <li v-for="item in category.items" :key="item.title"><a v-bind:href="item.link">{{ item.title }}</a></li>

                </ul>

                </li>

            </ul>

            <ul id="nav-links-right">

                <li><a href="javascript:void(0)">Sign Up</a></li>

                <li><a href="javascript:void(0)">Sign In</a></li>

                <li><a href="javascript:void(0)">Cart</a></li>

            </ul>

        </div>

    </div>


</template>



<script>

    // Import JavaScript affiliated with header. Component can interact with this JS if it were ever needed
    import '../assets/header.js'
    import categories from '../assets/data.json';
    var data = JSON.parse(JSON.stringify(categories))

    export default {
        name: 'VueHeader',
        data: function() {
            return {
                data
            }
        },

        mounted: function() {

            $('#header #nav').find('.dropdown').hide();
	
            $('ul li').hover(function() {
                $(this).children('.dropdown').stop().fadeIn(250);
            }, function() {
                $(this).children('.dropdown').stop().fadeOut(250)
            });
            
            $(function() {
                $('#hamburger').click(function() {
                    $('#nav').toggle()
                })
            })
        }
    }

</script>

<style>

    a {
        text-decoration: none !important;
    }

    #header {
        width: 100%;
        margin: 0;
        padding: 0;
    }

    #header-brand {
        width: 100%;
        text-align: center;
        margin: 0;
        padding: 0;
        background-color: #fafafb;
        box-shadow: 0 1px 0 rgba(12,13,14,0.1),0 1px 6px rgba(59,64,69,0.1);
    }

    #header-brand a {
        color: #555;
        display: inline-block;
        margin: 10px;
    }

    #header-brand a * {
        margin: 0;
    }

    #header-brand h1 {
        font-family: 'IBM Plex Serif', serif;
        font-size: 50px;
        -webkit-font-smoothing: antialiased;
        -moz-osx-font-smoothing: grayscale;
    }

    #header-brand p {
        font-family: 'Raleway', sans-serif;
        font-size: 15px
    }

    #nav {
        display: none;
        color: #555;
        font-family: 'Raleway', sans-serif;
        font-weight: 100;
        font-size: 14px;
        text-transform: uppercase;
        max-width: 90vw;
        margin: 20px auto;
    }

    #nav a {
        color: #555;
    }

    #nav .fas {
        text-align: center;
        width: 30px;
    }

    #nav ul {
        list-style: none;
        margin: 0;
        padding: 0
    }

    #nav ul:hover, 
    #nav ul:focus {
        cursor: default;
    }

    @media(min-width: 1023px) {

        #header-brand {
            width: 100%;
        }

        #header-brand a {
            /* width: 100%; */
            padding: 0 20px 10px;
            margin: 0 auto;
        }

        #brand-subtitle {
            text-align: center
        }
        #nav {
            display: block;
            height: 40px;
        }

        #nav ul li {
            font-size: 15px
        }
        
        #nav #nav-links-left {
            float: left;
        }
        
        #nav #nav-links-right {
            float: right;
        }

    }

    #nav li {
        display: block;
        margin: 10px 0;
        border-radius: 3px;
        padding: 10px;
        margin: 10px 0;
        position: relative;
        text-decoration: none;
        background: #e8e8e8;
        transition: all 0.25s ease;
        cursor: pointer;
    }

    #nav li .fas {
        -webkit-transition: -webkit-transform 0.25s ease-in-out;
        transition: transform 0.25s ease-in-out;
    }

    @media(min-width: 1023px) {
    #nav li {
        display: inline-block;
        margin: 0 10px;
    }

    }

    #nav li:hover, #nav li:focus {
        background: lightgray;
    }

    #nav li:hover .fas, #nav li:focus .fas {
        -webkit-transform: rotate(180deg);
        transform: rotate(180deg);
    }

    #nav .dropdown {
        border-top: 5px solid transparent;
        border-radius: 3px;
        position: relative;
        left: 0;
        min-width: 100%;
        margin-top: 10px;
    }

    #nav .dropdown li {
        margin: 10px auto;
        max-width: 90%;
        clear: both;
    }

    @media(min-width: 1023px) {
        #nav .dropdown li {
            min-width: 200px;
            margin: 0;
            text-align: left;
            border-radius: 0px
        }

        #nav .dropdown li:first-child {
            border-radius: 3px 3px 0 0
        }

        #nav .dropdown li:last-child {
            border-radius: 0 0 3px 3px
        }

    }

    @media(min-width: 1023px) {
        #nav .dropdown {
            position: absolute;
        }

    }

    #hamburger {
        width: 30px;
        height: 30px;
        margin-top: 15px;
        position: absolute;
        top: 25px;
        left: 5vw;
        border-radius: 3px;
        color: #555;
        background-color: #e8e8e8;
        display: flex;
        align-items: center;
        justify-content: center;
    }

    @media(min-width: 1023px) {
        #hamburger {
            display: none;
        }

    }

</style>
